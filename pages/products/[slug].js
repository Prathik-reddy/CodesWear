...
const Post = ({ addToCart, product, variants, buyNow }) => {
  ...
  const checkService = async () => {
    ...
    if (pinJson.includes(parseInt(pin))) {
      setservice(true)
      toast.success('Your PinCode is servicealbe!', {
      ...
      setservice(false);
      toast.error('Sorry! Your PinCode not is servicealbe', {
        position: "bottom-center",
        autoClose: 1000,
        hideProgressBar: false,
        closeOnClick: true,
  ...
  const refresh = (newSize, newColor) => {
    ...
    let url = `${process.env.HOST}/products/${variants[newColor][newSize]["slug"]}`
    window.location = url;
  }
